...
export default function DocumentsPage() {
  ...
  const counts = { expired: docs.filter(d=>statusOf(d)==='EXPIRED').length, warning: docs.filter(d=>statusOf(d)==='WARNING').length }
      ...
      <div style={{padding:'16px 20px 28px'}}>
        ...
        {counts.expired>0&&(
          <div style={{background:'#e8403a12',border:'1px solid #e8403a33',borderRadius:8,padding:'10px 16px',marginBottom:16,display:'flex',alignItems:'center',gap:10}}>
            ...
            <span style={{fontSize:12,color:'var(--danger)',fontWeight:500}}>{counts.expired} document{counts.expired>1?'s':''} expired — immediate action required</span>
          </div>
        )}
        ...
        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16}}>
          ...
          <div style={{display:'flex',gap:4}}>
            {[['all','All'],['expired','Expired'],['expiring','Expiring'],['ok','OK']].map(([v,l])=>(
              <button key={v} onClick={()=>setFilter(v)} style={{
                background:filter===v?'var(--bg-elevated)':'none',
                border:'1px solid',borderColor:filter===v?'var(--border)':'transparent',
          ...
          <div style={{flex:1}}/>
          <div style={{fontSize:12,color:'var(--text-muted)'}}>{filtered.length} documents</div>
          ...
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead>
              <tr style={{borderBottom:'1px solid var(--border-subtle)'}}>
                {['Document','Vendor','Type','Expiry Date','Expires In','Status',''].map(h=>(
              ...
              {filtered.map((d,i)=>{
                ...
                return (
                  <tr key={d.id} style={{borderBottom:'1px solid var(--border-subtle)',background:i%2===0?'transparent':'rgba(255,255,255,0.01)'}}>
                    ...
                    <td style={{padding:'11px 14px',color:'var(--text-muted)',fontSize:11,textTransform:'uppercase',letterSpacing:'0.05em'}}>{d.type||'—'}</td>
                    <td style={{padding:'11px 14px',color:'var(--text-secondary)',fontSize:12}}>{d.expiry_date||'—'}</td>
                    <td style={{padding:'11px 14px',fontSize:12,fontWeight:500,color:st==='EXPIRED'?'var(--danger)':st==='WARNING'?'var(--warning)':'var(--text-muted)'}}>
                    ...
                    <td style={{padding:'11px 14px',display:'flex',gap:6,alignItems:'center'}}>
                      {d.file_url&&<a href={d.file_url} target="_blank" style={{color:'var(--accent)',fontSize:11,textDecoration:'none',border:'1px solid var(--border)',borderRadius:5,padding:'3px 8px'}}>View</a>}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',zIndex:100,display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div style={{background:'var(--bg-surface)',border:'1px solid var(--border)',borderRadius:10,padding:24,width:420}}>
            ...
            <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
              ...
              <button onClick={save} disabled={saving} style={{background:'var(--accent)',color:'#fff',border:'none',borderRadius:6,padding:'8px 16px',cursor:'pointer',fontSize:13,fontWeight:500}}>{saving?'Uploading...':'Upload'}</button>
            </div>
          </div>
        </div>
      )}
