...
<body style="background:#0B0E14;margin:0;padding:40px 20px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
  <div style="max-width:580px;margin:0 auto">
    ...
    <div style="background:${color}18;border:1px solid ${color}40;border-radius:8px;padding:12px 16px;margin-bottom:24px">
    ...
    <div style="text-align:center;margin-bottom:32px">
      <a href="https://nordvenn.vercel.app/documents" style="display:inline-block;background:linear-gradient(135deg,#4C6FFF,#7AA2FF);color:#fff;text-decoration:none;padding:12px 28px;border-radius:8px;font-size:13px;font-weight:600">
        View in NordVenn →
      </a>
    </div>

    <div style="text-align:center;font-size:11px;color:#455065;border-top:1px solid #1a2130;padding-top:20px">
      NordVenn · Vendor Compliance Platform
    </div>
  </div>
</body>
</html>`
...
serve(async () => {
  try {
    ...
    if (docsErr) throw new Error('Docs error: ' + docsErr.message)
    ...
    if (!emails.length) return new Response(JSON.stringify({ error: 'No users found' }), { status: 200 })
    if (!docs?.length) return new Response(JSON.stringify({ sent: 0, msg: 'No docs with expiry' }), { status: 200 })
