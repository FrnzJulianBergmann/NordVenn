          ...
          <>
            ...
            <Section title="Plan — Dev Mode">
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => switchPlan('free')} style={{
                  flex: 1, padding: '9px', borderRadius: 7, cursor: 'pointer', fontSize: 12, fontWeight: 500,
                  border: '1px solid var(--border)',
                  background: !isPro ? 'var(--bg-elevated)' : 'none',
                  color: !isPro ? 'var(--text-primary)' : 'var(--text-muted)',
                }}>Free</button>
                <button onClick={() => switchPlan('pro')} style={{
                  flex: 1, padding: '9px', borderRadius: 7, cursor: 'pointer', fontSize: 12, fontWeight: 600,
                  border: 'none',
                  background: isPro ? 'linear-gradient(135deg,#4C6FFF,#7AA2FF)' : 'var(--bg-elevated)',
                  color: isPro ? '#fff' : 'var(--text-muted)',
                  boxShadow: isPro ? '0 2px 8px #4C6FFF40' : 'none',
                }}>Pro ✨</button>
              </div>
            </Section>
          </>
...
writeFileSync(resolve('app/(dashboard)/settings/page.tsx'), content, 'utf8')
console.log('✅ Settings page fixed & plan switcher added!')
